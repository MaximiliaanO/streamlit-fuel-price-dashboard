...
with st.container(border=True):
    ...
    st.altair_chart(charts.min_max_avg_chart(f_type="diesel", p_type="budget", min_scale=1.40, max_scale=2.0), use_container_width=True)

with st.container(border=True):
    st.subheader("Diesel prijzen bij premium tankstations:")
    st.altair_chart(charts.min_max_avg_chart(f_type="diesel", p_type="budget", min_scale=1.40, max_scale=2.0), use_container_width=True) # To change to premium gasstations.
